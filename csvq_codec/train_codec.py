...
def main():
    ...
    for i in range(cfg['num_experiments']):
        bitrate = (len(cfg[cfg['model_name']]['ch_mult']) + 1) * cfg[cfg['model_name']]['Groups'] * math.log2(cfg[cfg['model_name']]['codebook_size']) * 50
        cfg['model_tag'] = '0_{}_csvqcodec_scalable_{}kbps'.format(cfg['data_name'],bitrate*0.001)
    ...
    for i, input in enumerate(data_loader):
        ...
        if save_image and i % int((len(data_loader) * cfg['plot_interval']) + 1) == 0:
            ...
            with torch.no_grad():
                ...
                plot_spectrogram(input['stft_feat'][:4],output['recon_feat'][:4],img_path,evaluation,Bs=n)
            model.train(True)
    logger.safe(False)
    return
    ...
    with torch.no_grad():
        ...
        if save_image:
            ...
            plot_spectrogram(plot_batch['raw_feat'],plot_batch['recon_feat'],img_path,evaluation, 
                                                            Bs=len(cfg[cfg['model_name']]['ch_mult']) + 1)
    logger.safe(False)
    return
