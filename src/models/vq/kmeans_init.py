...
@torch.no_grad()
def codebook_init_forward_hook_gvq(self, input, output):
	""" initializes codebook from data """

	if (not self.training) or (self.codebook_initialized.item() == 1):
		return # prevent initialization during inference

	if self.verbose_init is True:
		print('initializing Group VQs with k-means++' if self.kmeans_init else 'initializing Group VQs by randomly choosing from z_e')

	z_q, _, _, _, _, z_e = output
	# z_e [bs, Group_size, T, codebook_dim]
	for i in range(self.num_vqs):
		z_e_i = z_e[:,i] # bs T codebook_dim
