...
# from visqol_meta import ViSQOL
...
class ViSQOL:
    def __init__(self, visqol_path="../visqol") -> None:
    ...
    def __call__(self, x, y):
        ...
        batch_visqol = []
        for b in range(bs):
            ref = x[b:b+1].unsqueeze(1).cpu()
            deg = y[b:b+1].unsqueeze(1).cpu()
            batch_visqol.append(self.metric(ref, deg, 16000))
        
        return torch.tensor(batch_visqol)
