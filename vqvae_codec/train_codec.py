...
def main():
    ...
    for i in range(cfg['num_experiments']):
        # model_tag_list = [str(seeds[i]), cfg['data_name'], cfg['model_name'], cfg['control_name'], 'depth_{}'.format(cfg[cfg['model_name']]['depth'])]
        # cfg['model_tag'] = '_'.join([x for x in model_tag_list if x])

        cfg['model_tag'] = '0_vqgan_based_codec_17.8k_test'
...
def runExperiment():
    ...
    # metric = Metric({'train': ['Loss','img_MSE','img_PSNR'], 'test': ['Loss','img_MSE','img_PSNR']})
    metric = Metric({'train': ['Loss','img_MSE','img_PSNR','audio_MSE','audio_PSNR'], 'test': ['Loss','audio_MSE','audio_PSNR','PESQ']})
    ...
    for epoch in range(last_epoch, cfg[cfg['model_name']]['num_epochs'] + 1):
        ...
        print('Epoch {} finish! Start to Test'.format(epoch))
...
def test(data_loader, model, metric, logger, epoch):
    ...
    with torch.no_grad():
        ...
        for i, input in enumerate(data_loader):
            ...
            if i % 100 == 0:
                info = {'info': ['Model: {}'.format(cfg['model_tag']), 'Test Epoch: {}({:.0f}%)'.format(epoch, 100. * i / len(data_loader))]}
