...
class LIBRISPEECH(Dataset):
    ...
    def __init__(self, root, split, transform=None) -> None:
        ...
        if not check_exists(self.processed_folder):
            self.process()
        else: print("processed data exist!")

        id, self.data = load(os.path.join(self.processed_folder, '{}.pt'.format(self.split)),
                                          mode='pickle')
        self.other = {'id': id}
    ...
    def __getitem__(self, idx): # return ith audio waveform data 
        ...
        if self.transform is not None:
            input['data'] = self.transform(input['raw'])
        return input
    ...
    def process(self):
        ...
        train_id, train_data, test_id, test_data  = self.make_data()

        save((train_id,train_data), os.path.join(self.processed_folder, 'train.pt'), mode='pickle')
        save((test_id,test_data), os.path.join(self.processed_folder, 'test.pt'), mode='pickle')
    ...
    def make_data(self):
        ...
        # train_audio_files = train_audio_files[:int(len(train_audio_files)*0.7)]
        ...
        def audio_load(files):
            ...
            for f in files:
                ...
                data.append(wf)
                # trans_data.append(sdct_torch(wf, frame_length=1024, frame_step=256).squeeze().permute(1,0).contiguous())
            return data

        test_data = audio_load(test_audio_files)
        train_data = audio_load(train_audio_files)
