...
def fetch_dataset(data_name):
    ...
    if data_name in ['LIBRISPEECH']:
        ...
        dataset['train'].transform = utils.STDCT()
        dataset['test'].transform = utils.STDCT()
    else:
...
def input_collate(batch):
    ...
    #     output = {key: [] for key in batch[0].keys()}  # output = {'data':[.....], 'trans_data':[.....]}
    #     for b in batch:
    #         for key in b:
    ...
    # else: 
        output = {'raw':[],'data':[]}
        for b in batch:
            output['raw'].append(b['raw'])
            output['data'].append(b['data'].squeeze().permute(1,0).contiguous())
        return output
