import sys
sys.path.append("/Users/tracy/Desktop/Audio_Codec/csvq-codec-baseline")

from models.convolution import ConvEncoderLayer, ConvDecoderLayer, Convolution2D
from models.tfs import TCM, RNNFilter
from models.swin import SwinTLayer, PatchEmbed, PatchDeEmbed, PatchMerging, PatchSplit
...
class BaseCrossScaleDecoder(nn.Module):
    def __init__(self, in_dim: int, h_dims: list, max_streams: int, fuse_net: bool) -> None:
        super().__init__()
    ...
    def pre_fuse(self, enc, dec, idx):
        return self.pre_fuse_net[idx](torch.cat([enc,dec],dim=1))
            
    def post_fuse(self, residual_q, dec, idx):
    ...
    def csvq_layer(self, enc, dec, idx, vq):
        if self.fuse_net:
            ...
            print(idx, "pre_fuse_out: ", residual.shape)
            residual_q, vq_loss = vq(residual)
            dec_refine = self.post_fuse(residual_q, dec, idx)
            print(idx, "post_fuse_out: ", dec_refine.shape)

        else:
            residual = self.res_pre_fuse(enc, dec)
            print(idx, "res_pre_fuse_out: ", residual.shape)
            residual_q, vq_loss = vq(residual)
            dec_refine = self.res_post_fuse(residual_q, dec)
            print(idx, "res_post_fuse_out: ", dec_refine.shape)
...
class SwinCrossScaleDecoder(BaseCrossScaleDecoder):
    def __init__(self, 
                 ...
                 max_streams: int = 6, 
                 fuse_net: bool = False) -> None:
        super().__init__(in_dim, h_dims, max_streams, fuse_net)
...
class ConvCrossScaleDecoder(BaseCrossScaleDecoder):
    def __init__(self, 
        ...
        if self.fuse_net:
            print("Use Fuse Merge Net")
            self.pre_fuse_net, self.post_fuse_net = nn.ModuleList(), nn.ModuleList()
            for i in range(max_streams-1):
                self.pre_fuse_net.append(
                    nn.Sequential(*[
                        # Convolution2D(h_dims[i]*2, h_dims[i], 3, 1, causal=True),
                        # Convolution2D(h_dims[i], h_dims[i], 3, 1, causal=True),
...
if __name__ == "__main__":
    ...
    # codec = SwinCrossScaleCodec(overlap=2, proj=2, fuse_net=False)
    # outputs = codec.train_one_step(x=torch.randn(1,47920),
    #                      x_feat=torch.randn(1,192,600,2),
    #                      streams=6)
    # x = enc_hs[-1]
